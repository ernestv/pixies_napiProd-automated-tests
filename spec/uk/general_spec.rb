...
#      browser.a(:class, "button hide_for_mobile").when_present.click
    ...
    context "Radio" do
      ...
      it "Browse by Genre" do
        ...
        browser.div(:text, "Pop").when_present.click
        sleep 1
        expect(browser.url).to include("napster.com/radio/genres/genre/pop")
        expect(browser.div(:class, "genre-list")).to exist
        expect(browser.div(:class, "radio-station-list scroll-under-player")).to exist
        sleep 1
    end
...
#      expect(browser.url) .to include ("us.napster.com/availability")
      expect(browser.url) .to include ("napster.com/quickhelp") #http://www.napster.com/help <- for US
